...
let step = 0; // 0 - draw shape, 1 - processing, 2 - finished
let iterations = 0;
...
let previousGrid = Array.from({ length: gridSize }, () => Array(gridSize).fill(0));
...
function hitOrMiss(grid, element) {
...
function union(gridArr, hitOrMissArr) {
...
function reset() {
    ...
    previousGrid = Array.from({ length: gridSize }, () => Array(gridSize).fill(0));
    step = 0;
    iterations = 0;
    document.getElementById("iterations-count").innerText =
        iterations.toString();
...
function renderStructuringElements() {
    const structuringContainer = document.getElementById("structuring-elements");
